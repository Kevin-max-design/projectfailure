...
async function processDocument(documentId: string, adminSupabase: any) {
  console.log(`\n${'─'.repeat(60)}`);
  ...
  try {
    ...
    const { DocumentProcessingPipeline } = await import('../src/lib/extraction/pipeline');
    ...
    if (data) {
      const nonEmptyCategories = Object.entries(data)
        .filter(([k, v]) => {
          ...
          if (typeof v === 'string') return (v as string).length > 0;
          return v != null;
        })
        .map(([k]) => k);
