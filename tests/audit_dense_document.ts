...
async function runOcr(filePath: string, engine: string): Promise<OcrResult> {
...
function checkSectionCoverage(fullText: string): Array<{ section: string; found: boolean; matchedPattern?: string }> {
...
function computeStats(result: OcrResult) {
...
async function checkHealth(): Promise<{ status: string; engine: string; tesseractVersion: string | null; paddleAvailable: boolean }> {
  const resp = await fetch(`${OCR_SERVICE_URL}/health`);
  return resp.json();
}

async function auditFixture(filePath: string, engines: string[]) {
  const fileName = path.basename(filePath);
  console.log(`\n${'═'.repeat(80)}`);
  console.log(`FIXTURE: ${fileName}`);
  console.log(`${'═'.repeat(80)}`);

  const results: Record<string, { stats: ReturnType<typeof computeStats>; coverage: ReturnType<typeof checkSectionCoverage>; timeMs: number }> = {};
...
async function main() {
  ...
  const allResults: Record<string, Record<string, ReturnType<typeof computeStats>>> = {};
