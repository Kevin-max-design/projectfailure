...
exports.LocalQAProvider = void 0;
const openai_1 = __importDefault(require("openai"));
class LocalQAProvider {
    constructor() {
...
Format your final output as a JSON object matching this schema:
{
  ...
  "citations": [
    ...
    retrieveDeterministically(question, contextChunks, structuredRecords) {
        ...
        const normalizeMedicalTerm = (w) => {
            let term = w.toLowerCase();
            if (term.includes('hemo') || term.includes('haemo'))
                return 'hemoglob';
            return term;
        };
        const queryWords = q
        ...
        const matchesKeyword = (target, query) => {
        ...
        const getCitationForRecord = (record) => {
        ...
        const matchingRecords = structuredRecords.filter((r) => {
            const recordName = (r.name || r.medicine_name || r.test_name || r.title || '').toLowerCase();
            if (!recordName)
                return false;
            // Exact word match or substring
            return q.includes(recordName) || recordName.split(/\s+/).some((word) => word.length > 3 && q.includes(word));
        });
        ...
        // 7. General search matching raw context chunks (if no structured matched)
            ...
            if (matchedChunk) {
                ...
                if (!snippet && paragraphs.length > 0)
...
exports.LocalQAProvider = LocalQAProvider;
