...
function getCookieHeader(session: any) {
...
async function runValidation() {
  ...
  const existingA = (listData?.users as any[])?.find((u: any) => u.email === emailA);
  if (existingA) {
    console.log(`  Deleting existing User A (${existingA.id})`);
    await adminSupabase.auth.admin.deleteUser(existingA.id);
  }
  const existingB = (listData?.users as any[])?.find((u: any) => u.email === emailB);
  if (existingB) {
  ...
  if (errCreateA || !userA) {
    throw new Error(`Failed to create User A: ${errCreateA?.message}`);
  }
  ...
  if (errPatA || !patientA) {
    throw new Error(`Failed to create Patient A: ${errPatA?.message}`);
  }
  ...
  if (errCreateB || !userB) {
    throw new Error(`Failed to create User B: ${errCreateB?.message}`);
  }
  ...
  if (errPatB || !patientB) {
    throw new Error(`Failed to create Patient B: ${errPatB?.message}`);
  }
  ...
  if (errLoginB || !sessionB) {
    throw new Error(`Failed to log in as User B: ${errLoginB?.message}`);
  }
  ...
  const uploadDoc = async (filename: string, buffer: Buffer, mime: string, category: string) => {
    ...
    return data.documentId || data.document?.id || data.id;
  };
  ...
  const auditDb = async (docId: string, label: string) => {
  ...
  const checkLabs = async (docId: string, label: string, expectedLabs: string[]) => {
    ...
    if (error || !labs) {
      throw new Error(`Failed to fetch lab results for ${label}: ${error?.message}`);
    }

    console.log(`  [${label}] Extracted structured lab records count: ${labs.length}`);
    const names = labs.map((l: any) => l.test_name.toLowerCase());
    console.log(`  [${label}] Labs: ${labs.map((l: any) => `${l.test_name}: ${l.value} ${l.unit || ''}`).join(', ')}`);

    for (const expected of expectedLabs) {
      const found = names.some((n: string) => n.includes(expected.toLowerCase()));
      if (!found) {
  ...
  const verifyReviewApi = async (docId: string, label: string) => {
    ...
    const totalReadable = ext.coverageMetrics?.totalReadableTextBlocks || 0;
    const mapped = ext.coverageMetrics?.mappedStructuredBlocks || 0;
    const unmapped = ext.coverageMetrics?.unmappedBlocks || 0;
    const unreadable = ext.coverageMetrics?.unreadableBlocks || 0;
  ...
  const hasEventForDoc = timelineData.some((e: any) => e.source_document_id === docIdA);
  if (!hasEventForDoc) {
