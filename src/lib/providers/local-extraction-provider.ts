...
export class LocalExtractionProvider implements MedicalExtractionProvider {
...
"""`;
          ...
          extractionResult = MedicalExtractionSchema.parse(parsed);
          llmSuccess = true;
  ...
  private parseDeterministically(ocrText: string, category?: DocumentCategory): MedicalExtraction {
    ...
    const isLabReport = hasLabKeywords || hasLabResultMarkers;
    ...
    } else if (isLabReport && !hasBillingKeywords) {
      docType = 'LAB_REPORT';
      classConf = 0.95;
    } else if (hasPrescriptionRx && !hasBillingKeywords) {
    ...
    if (category && category !== 'Auto Detect') {
      ...
      if (supported.includes(catUpper)) { docType = catUpper; }
      else if (category === 'Prescription') docType = 'PRESCRIPTION';
      else if (category === 'Lab Report') docType = 'LAB_REPORT';
      else if (category === 'Discharge Summary') docType = 'DISCHARGE_SUMMARY';
      else if (category === 'Imaging Report') docType = 'IMAGING_REPORT';
      else if (category === 'Medical Certificate') docType = 'MEDICAL_CERTIFICATE';
    ...
    for (const line of lines) {
      ...
      if (lineLower.includes('age:') || lineLower.startsWith('age ')) {
        ...
        const val = parts[1]?.trim() || line.replace(/age/i, '').trim();
      ...
      if (lineLower.includes('gender:') || lineLower.includes('sex:')) {
        const val = line.split(/gender:|sex:/i)[1]?.trim();
        if (val) {
      ...
      if (lineLower.includes('mrn:') || lineLower.includes('patient id:') || lineLower.includes('mrn number:')) {
        const val = line.split(/mrn:|patient id:/i)[1]?.trim();
        if (val) {
    ...
    for (const line of lines) {
      ...
      // Match keywords in words using regex boundaries to avoid false positives (e.g. concentration)
      const hasMedKeyword = /\b(tab|tablet|cap|capsule|once|daily|rx|mg|ml|mcg|syp|inj|ointment)\b/i.test(cleanLineLower);
      
      // Additional structural match: contains numbers + medication indicators
      if (hasMedKeyword && cleanLine.length < 120) {
    ...
    const globalLabRegex = new RegExp(
      `([^\\n\\|;\\(\\)]+?)\\s*[|\\*\\-\\–\\—\\s]*(\\d+(?:[.,]\\d+)?)\\s*[.\\-\\s]*(${allUnitPatterns})(?![a-zA-Z0-9])`,
      'gi'
    );

    const isCleanUnit = (unitStr: string): boolean => {
      const clean = unitStr.trim();
      const standardLiterals = [
        'gm%', 'gms%', 'g/dl', 'g/dL', 'mg/dl', 'mg/dL', 'u/l', 'U/L', 'iu/l', 'IU/L', 
        'fl', 'pg', '/cumm', '/ul', '/uL', 'vol%', 'millions/cumm', '%'
      ];
      if (standardLiterals.includes(clean)) return true;
      
      const cleanMultiplierRegex = /^(?:x|10\^)?10\^\d+\/(?:uL|ul|cumm)$/i;
      if (cleanMultiplierRegex.test(clean)) return true;
    ...
    return MedicalExtractionSchema.parse(result);
  }
}
